...
def _shared_boundary_mask(label_map: np.ndarray) -> np.ndarray:
...
def build_outline_image_from_labels(
    label_map: np.ndarray,
    line_width: int,
    line_color_hex: str = "#444444",
) -> np.ndarray:
    h, w = label_map.shape
    canvas = np.full((h, w, 3), 255, dtype=np.uint8)
    boundary = _shared_boundary_mask(label_map)
    if line_width > 1:
        k = max(1, int(line_width))
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
        boundary = cv2.dilate(boundary, kernel, iterations=1)
    color = _parse_hex_color(line_color_hex)
    canvas[boundary > 0] = np.array(color, dtype=np.uint8)
    return canvas
...
def build_numbered_image_from_labels(
    label_map: np.ndarray,
    numbers: list[RegionNumber],
    line_width: int,
    font_scale: float,
    ...
    canvas = build_outline_image_from_labels(label_map, line_width=line_width, line_color_hex=line_color_hex)
    text_color = _parse_hex_color(line_color_hex)
    for region in numbers:
        if region.number_skipped or region.number_position_xy is None:
            continue
        if region.uses_external_label and region.leader_line_start_xy is not None and region.leader_line_end_xy is not None:
            cv2.line(canvas, region.leader_line_start_xy, region.leader_line_end_xy, text_color, 1, cv2.LINE_AA)
            cv2.line(canvas, region.leader_line_end_xy, region.number_position_xy, text_color, 1, cv2.LINE_AA)
        x, y = region.number_position_xy
        text = str(region.color_label + 1)
        (tw, th), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, 1)
        tx = int(x - tw / 2)
        ty = int(y + th / 2)
        cv2.putText(canvas, text, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, font_scale, text_color, 1, cv2.LINE_AA)
    return canvas
