...
def build_edge_strength_map(rgb: np.ndarray) -> np.ndarray:
    ...
    laplacian = normalize01(np.abs(cv2.Laplacian(gray, cv2.CV_32F, ksize=3)))

    edge = normalize01(0.24 * sobel3 + 0.20 * sobel5 + 0.18 * canny_fine + 0.18 * canny_coarse + 0.20 * laplacian)
